...
export function Sidebar({ isOpen, onOpenChange, currentView, onViewChange }: SidebarProps) {
  const navItems = [
  ...
  return (
    <>
      {/* Mobile overlay */}
      {isOpen && <div className="fixed inset-0 bg-black/50 md:hidden z-40" onClick={() => onOpenChange(false)} />}
      ...
      >
        ...
        {/* Logo */}
        <div className="hidden md:flex h-16 items-center justify-center border-b border-sidebar-border">
          <div className="flex items-center gap-2">
            <div className="w-8 h-8 rounded-lg bg-sidebar-primary/20 flex items-center justify-center">
              <svg className="w-5 h-5 text-sidebar-primary" fill="currentColor" viewBox="0 0 24 24">
                <path d="M12 2L15.09 8.26H22L16.95 12.91L19.09 19.16L12 14.5L4.91 19.16L7.05 12.91L2 8.26H8.91L12 2Z" />
              </svg>
            </div>
            <span className="font-bold text-sidebar-foreground text-sm">GoldShop</span>
          </div>
        </div>
        ...
        <div className="absolute bottom-0 left-0 right-0 p-4 border-t border-sidebar-border">
          <p className="text-xs text-sidebar-foreground/60 text-center">Invoice Management v1.0</p>
        </div>
      </aside>
    </>
  )
